...
# from network.pix2pix.ImagePool import ImagePool
from inference.mlmodels.two_pix2pix.network.pix2pix_model import Pix2PixModel
import inference.mlmodels.two_pix2pix.network.networks as networks
...
class TwoPix2PixModel:
    ...
    def initialize(self, opt, silent=True):
        # copy from BaseModel
        self.opt = opt
        self.gpu_ids = opt.gpu_ids
        self.isTrain = opt.isTrain
        self.Tensor = torch.cuda.FloatTensor if self.gpu_ids else torch.Tensor
        self.save_dir = utils.get_two_pix2pix_model_path()
    ...
    def infer(self, image_tensor):
        self.image_tensor = image_tensor.cuda(self.gpu_ids[0], non_blocking=True)
        self.real_A = Variable(self.image_tensor)
        self.fake_B = self.seg_netG(self.real_A)
        fake_B = (self.fake_B + 1.0)/2.0
        input_A = (self.real_A + 1.0)/2.0
        self.fake_C = (fake_B * input_A) * 2.0 - 1
    ...
    def get_current_visuals(self):
        if self.isTrain:
            ...
            fake_D = utils.tensor2im(self.fake_D.data)
            real_D = utils.tensor2im(self.real_D.data)
            fake_C = utils.tensor2im(self.fake_C.data)
