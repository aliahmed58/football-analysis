...
def edge_map_from_homography(homography, binary_court, edge_map_resolution):
    ...
    for i in range(n):
        idx1, idx2 = int(line_segment_indexes[i][0]), int(line_segment_indexes[i][1])
...
class Camera:
    ...
    def project_point_on_topview(point, h, f_w=1, f_h=1):
        ...
        assert q[2] != 0.0
        projected_x = np.rint(q[0] / q[2]).astype(np.int64)
        projected_y = np.rint(q[1] / q[2]).astype(np.int64)
