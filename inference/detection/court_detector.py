# ---------------------------------------------
# Module that helps in detecting the football
# court and draws edges on the frame, used to 
# get boundaries and then for 2d mapping of objs
# ---------------------------------------------
...
from inference.mlmodels.two_pix2pix.network.two_pix2pix_model import TwoPix2PixModel
from inference.mlmodels.two_pix2pix.test_options import opt
...
if __name__ == '__main__':
    ...
    image_filelist = glob.glob(f"{utils.get_project_root()}/test/test.jpg")
    estimator = CourtDetector((1280, 720))
    ...
    for image_filename in image_filelist:
        ...
        utils.save_image(output_image, f"{utils.get_project_root()}/test/{image_filename.split('/')[-1]}")
